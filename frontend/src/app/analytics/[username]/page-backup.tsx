...
export default function AnalyticsPage() {
  ...
  console.log('🎬🎬🎬 === COMPONENT RENDER ===', renderId)
  ...
  console.log('🎬🎬🎬 === RENDER END ===', renderId)
  ...
  // REMOVED: Progressive loading no longer needed with new backend behavior
  // Backend now returns complete data instantly for existing profiles
  ...
  const analyzeProfile = useCallback(async (targetUsername: string, isBackgroundRefresh: boolean = false) => {
    const callStack = new Error().stack?.split('\n').slice(1, 5).join('\n') || 'No stack'
    console.log('🔄🔄🔄 === ANALYZE PROFILE CALLED ===')
    ...
    // PREVENT CONCURRENT CALLS - this is the key fix!
    if (isAnalyzing) {
      console.log('⏸️⏸️⏸️ ALREADY ANALYZING - SKIPPING DUPLICATE CALL')
      console.log('⏸️ Current isAnalyzing state:', isAnalyzing)
      return
    }
    ...
    // Only show full loading state if this is not a background refresh
    if (!isBackgroundRefresh) {
      console.log('🔃 Setting loading to TRUE')
      setLoading(true)
    }
    console.log('🧹 Clearing error and debugInfo')
    setError(null)
    setDebugInfo(null)
    
    try {
      console.log('📡 Calling NEW simplified API (instant results for existing profiles)...')
      // NEW BACKEND: /basic now returns complete data with AI insights for existing profiles
      const result = await instagramApiService.getBasicProfile(targetUsername)
      ...
      // SIMPLIFIED: No more progressive loading needed - backend returns complete data instantly for existing profiles
      console.log('✅ Profile loaded successfully with complete data!')
      ...
      if (aiStatus === 'pending') {
        ...
        // REMOVED: Old auto-refresh call replaced with progressive loading
      } else if (aiStatus === 'completed') {
        console.log('✅ AI analysis completed - no auto-refresh needed')
        // REMOVED: Old timer cleanup - no longer needed with progressive loading
      }
      ...
      console.log('🔚🔚🔚 === ANALYZE PROFILE FINALLY ===')
      console.log('🔚 Setting loading to FALSE')
      setLoading(false)
      console.log('🔚 Setting isAnalyzing to FALSE (releasing lock)')
      setIsAnalyzing(false) // CRITICAL: Reset the lock to allow future calls
      console.log('🔚 Final state - loading:', loading, 'isAnalyzing will be:', false)
  ...
  // REMOVED: Old auto-refresh system replaced with progressive loading
  ...
  // Auto-load analytics when component mounts - with proper concurrency control
  useEffect(() => {
    ...
    console.log('🚀 Component state snapshot:', { username, isAnalyzing, loading, error })
    
    if (username && typeof username === 'string' && !initialLoadTriggered) {
      console.log('🌐 CONDITIONS MET - scheduling analyzeProfile call in 100ms')
      console.log('🌐 Effect ID:', effectId)
      
      setInitialLoadTriggered(true) // Prevent multiple calls
      
      // Small delay to prevent rapid-fire calls
      const timeoutId = setTimeout(() => {
        console.log('⏰ TIMEOUT FIRED - calling analyzeProfile for effect', effectId)
        analyzeProfile(username)
      }, 100)
      
      // Cleanup timeout on unmount/re-run
      return () => {
        console.log('🧹 USEEFFECT CLEANUP - clearing timeout for effect', effectId)
        clearTimeout(timeoutId)
      }
    } else {
      console.log('🚫 CONDITIONS NOT MET:')
      console.log('🚫   - username exists:', !!username)
      console.log('🚫   - username is string:', typeof username === 'string')  
      console.log('🚫   - initial load not triggered yet:', !initialLoadTriggered)
    }
    console.log('🚀🚀🚀 === USEEFFECT END ===', effectId)
  }, [username, analyzeProfile, initialLoadTriggered]) // FIXED: Include all dependencies

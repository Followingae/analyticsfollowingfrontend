  ...
  const handleAction = async (action: string, user: User) => {
    switch (action) {
      ...
      case 'send-password-reset':
        try {
          await superadminService.sendPasswordReset(user.id);
        ...
        break;
      case 'manage-credits':
        const action = prompt('Enter "add" to add credits or "remove" to remove credits:');
        if (action === 'add' || action === 'remove') {
          const amount = prompt(`Enter amount of credits to ${action}:`);
          if (amount && !isNaN(Number(amount))) {
            const reason = prompt('Enter reason:');
            if (reason) {
              try {
                if (action === 'add') {
                  await superadminService.addCredits(user.id, Number(amount), reason);
                } else {
                  await superadminService.removeCredits(user.id, Number(amount), reason);
                }
                toast.success(`Credits ${action === 'add' ? 'added' : 'removed'} successfully`);
                fetchUsers();
              } catch (error) {
                toast.error(`Failed to ${action} credits`);
              }
            }
          }
        }
                      ...
                      <TableCell className="text-right">
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end" className="w-[180px]">
                            ...
                            <DropdownMenuItem onClick={() => handleAction('manage-credits', user)}>
                            ...
                            {user.is_admin_managed && (
                              <DropdownMenuItem onClick={() => {
                                ...
                                if (password) {
                                  superadminService.setPassword(user.id, password)

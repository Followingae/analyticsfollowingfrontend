 ...
 *
 * Cards are grouped under two section headings — "Partnerships" and
 * "Content signals" — so the eye can rank ~9 otherwise equally-weighted cards.
 * No card is removed and every block keeps its own isPresent gate; a section
 * heading only renders when at least one card beneath it is present.
 */
export function ContentTabV2({ data }: { data: CreatorAnalyticsV2 }) {
  ...
  const hasBrands = isPresent(brands) && brands.brands.length > 0
  const hasPaidVsOrganic = isPresent(self_declared_ads) && self_declared_ads.declared_ad_posts > 0
  const hasPartnerships = hasBrands || hasPaidVsOrganic
  ...
  // Only explain the confidence levels that actually appear on this creator.
  const confidencesPresent: BrandConfidence[] = hasBrands
    ? (["collab", "tagged", "mention"] as BrandConfidence[]).filter((c) =>
        brands.brands.some((b) => b.confidence === c),
      )
    : []
  ...
  return (
    <div className="space-y-10">
          ...
          {hasBrands && (
            <Card>
              ...
              <CardContent>
                      ...
                      <div key={b.username}
                        ...
                        <Tooltip>
                          ...
                          <TooltipContent className="max-w-xs">
                            {CONFIDENCE_EXPLAIN[b.confidence]}
                ...
                {/* Visible legend: the same meaning the per-badge tooltip carries,
                    so it is not lost on touch devices. */}
                {confidencesPresent.length > 0 && (
                  <dl className="mt-4 flex flex-col gap-1.5 border-t pt-3 text-xs text-muted-foreground sm:flex-row sm:flex-wrap sm:gap-x-4">
                    {confidencesPresent.map((c) => (
                      <div key={c} className="flex items-center gap-1.5">
                        <Badge variant="outline"
                               className={`shrink-0 text-[10px] ${CONFIDENCE_STYLES[c]}`}>
                          {BRAND_CONFIDENCE_LABELS[c]}
                        </Badge>
                        <dd>{CONFIDENCE_EXPLAIN[c]}</dd>
                      </div>
                    ))}
                  </dl>
                )}
      ...
      {hasContentSignals && (
        <section className="space-y-6">
          <SectionHeading>Content signals</SectionHeading>
